...
def timeline(events: Iterable[Event]) -> list[Event]:
    ...
    return sorted(events, key=lambda e: (e.get("@timestamp") is None, e.get("@timestamp") or ""))
...
def user_activity(events: Iterable[Event]) -> dict[str, dict[str, Any]]:
    ...
    # make JSON-friendly
    return {
        name: {**u, "actions": dict(u["actions"]), "source_ips": sorted(u["source_ips"])}
...
def top_talkers(events: Iterable[Event], limit: int = 20) -> list[dict[str, Any]]:
    ...
    for ev in events:
        if "network" not in ev:
            continue
        key = (_get(ev, "source.ip"), _get(ev, "destination.ip"), _get(ev, "destination.port"))
        agg[key]["bytes"] += _get(ev, "network.bytes", 0) or 0
        agg[key]["flows"] += 1
    rows = [{"source": k[0], "destination": k[1], "port": k[2], **v} for k, v in agg.items()]
...
def correlate_identities(events: Iterable[Event]) -> list[dict[str, Any]]:
    ...
    for name, provs in by_user.items():
        if len(provs) > 1:
            hits.append({"kind": "identity", "value": name, "providers": sorted(provs)})
    for ip, provs in by_ip.items():
        if len(provs) > 1:
            hits.append({"kind": "source_ip", "value": ip, "providers": sorted(provs)})
    return sorted(hits, key=lambda h: (-len(h["providers"]), h["kind"]))


def exfil_indicators(
    events: Iterable[Event],
    byte_threshold: int = 100_000_000,
    s3_events=("GetObject", "ListObjects", "ListObjectsV2"),
) -> list[Event]:
